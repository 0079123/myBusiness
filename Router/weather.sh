...
#4755
serverchan_sckey4=`7002-0b96187d49c1a354e9f6c7be41853f70`
#cd /tmp/

curl -s "http://api.map.baidu.com/telematics/v3/weather?location=福州&output=json&ak=6dd11fd43e63dde2abc6bc9564aa37b3" >/tmp/packet.json
#get sourse data
./json.sh < /tmp/packet.json | grep -E "weather_data|temperature" | awk 'NR<7{print $2,$3,$4}' >jsond.json
#format sourse data
...
curl -s "https://pushbear.ftqq.com/sub?sendkey=$serverchan_sckey4&text=$title&desp=$content"

#curl -s "http://sc.ftqq.com/$serverchan_sckey4.send?text=$title" -d "&desp=$content"
